...
const workspaceId = computed(() => route.params.workspace_id || activeWorkspace.value?.id || workspaces.value?.[0]?.id)
...
<template>
    <div>
        ...
        <div v-if="isCreateWorkspaceModalOpen" class="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-black/60 backdrop-blur-sm" @click.self="isCreateWorkspaceModalOpen = false">
          <div class="neo-card bg-gradient-to-b from-white to-gray-50 dark:from-[#2A2A2D] dark:to-[#222224] w-full max-w-md max-h-full rounded-xl flex flex-col overflow-hidden shadow-2xl" role="dialog" aria-modal="true">
            ...
            <div class="px-6 py-4 flex justify-between items-center shadow-[0_2px_10px_rgba(0,0,0,0.05)] dark:shadow-[0_2px_10px_rgba(0,0,0,0.2)] z-10 relative shrink-0">
              <div class="flex items-center gap-2 text-main dark:text-white">
                <Icon name="heroicons:squares-2x2" class="w-6 h-6 text-primary dark:text-primary" />
                <h2 class="text-xl font-bold">Nouvel Espace de Travail</h2>
              </div>
              <button @click="isCreateWorkspaceModalOpen = false" class="text-secondary hover:text-main dark:text-gray-400 dark:hover:text-white transition-colors p-1 rounded-md hover:bg-black/5 dark:hover:bg-white/5">
                <Icon name="heroicons:x-mark" class="w-6 h-6" />
              </button>
            </div>
            ...
            <div class="p-6 flex-1 overflow-y-auto custom-scrollbar">
              <div class="flex flex-col gap-5">
                <div v-if="createWorkspaceError" class="p-3 bg-red-50 text-red-600 dark:bg-red-900/20 dark:text-red-400 rounded-lg text-sm font-medium border border-red-100 dark:border-red-900/30">
                  {{ createWorkspaceError }}
                </div>
                
                <div>
                  <label class="block text-sm font-bold text-main dark:text-gray-300 mb-1.5">Nom de l'espace <span class="text-red-500">*</span></label>
                  <input 
