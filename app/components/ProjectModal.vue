...
const colorGradientClass = computed(() => {
  const c = projectColor.value || 'blue'
  const map: Record<string, string> = {
    'purple': 'from-purple-500/10 to-purple-500/5 dark:from-purple-900/20 dark:to-purple-900/5',
    'blue': 'from-blue-500/10 to-blue-500/5 dark:from-blue-900/20 dark:to-blue-900/5',
    'green': 'from-emerald-500/10 to-emerald-500/5 dark:from-emerald-900/20 dark:to-emerald-900/5',
    'rose': 'from-rose-500/10 to-rose-500/5 dark:from-rose-900/20 dark:to-rose-900/5',
    'amber': 'from-amber-500/10 to-amber-500/5 dark:from-amber-900/20 dark:to-amber-900/5',
    'slate': 'from-slate-500/10 to-slate-500/5 dark:from-slate-900/20 dark:to-slate-900/5',
  }
...
<template>
  <ClientOnly>
    ...
    >
      <div v-if="isOpen" class="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm" @click="close"></div>
    </Transition>
    ...
    >
      <div v-if="isOpen" class="fixed inset-0 z-[110] flex items-center justify-center p-4 sm:p-6 overflow-y-auto" @click="close">
        <div 
          class="bg-white dark:bg-[#1D1D1D] rounded-xl shadow-2xl w-full max-w-4xl flex flex-col max-h-full border border-gray-200 dark:border-gray-800 overflow-hidden relative"
          role="dialog"
          aria-modal="true"
          @click.stop
        >
        <!-- Dynamic Header Banner (Hidden now as per user request to keep flat design) -->
        <!-- <div class="h-24 md:h-32 w-full bg-gradient-to-b shrink-0 absolute top-0 left-0 right-0 z-0 pointer-events-none" :class="colorGradientClass"></div> -->

        <!-- Header -->
        <header class="flex items-start justify-between px-6 pt-6 pb-4 shrink-0 z-10 relative">
          ...
          <div class="flex items-center gap-1.5 shrink-0 bg-white/80 dark:bg-[#1D1D1D]/80 backdrop-blur-md p-1 rounded-lg border border-black/5 dark:border-white/5 shadow-sm">
        ...
        <div class="flex-1 flex flex-col md:flex-row overflow-hidden z-10">
          ...
          <!-- Main Content (Left) -->
          <div class="flex-1 overflow-y-auto custom-scrollbar p-6 md:border-r border-form-border dark:border-gray-800 flex flex-col gap-8 bg-white dark:bg-[#1D1D1D]">
          ...
          <!-- Sidebar (Right) -->
          <div class="w-full md:w-[280px] shrink-0 flex flex-col overflow-y-auto custom-scrollbar bg-gray-50/50 dark:bg-[#151515] p-6 gap-8 z-10">
            ...
            <!-- Enhanced Progress -->
            <div>
              <div class="bg-card dark:bg-[#222224] rounded-xl p-5 border border-form-border dark:border-gray-800 shadow-sm flex flex-col gap-4">
