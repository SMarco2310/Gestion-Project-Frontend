...
<template>
  <div :class="[
    ...
    <div v-show="!isCollapsed" class="flex-1 flex flex-col px-4 pb-4 overflow-y-auto custom-scrollbar gap-2">
      ...
      <!-- Inline Create Form -->
      <div v-if="isCreating" ref="createFormRef" class="bg-[#24334E] rounded-lg p-3 mb-2 border border-blue-400 shadow-sm flex flex-col gap-2 shrink-0">
        <textarea 
        ...
        <div class="flex items-center justify-between mt-1">
          <div class="flex items-center gap-2.5">
            ...
            <CustomSelect 
              ...
              placeholder="Projet"
              buttonClass="text-[11px] font-medium text-gray-300 bg-[#16233B] border border-white/10 rounded px-1.5 py-1 focus:outline-none focus:border-blue-400 cursor-pointer w-24 flex justify-between items-center"
              dropdownClass="w-32 mt-1 bg-[#1D1D1D] rounded-xl shadow-lg border border-gray-800 z-50 absolute left-0 top-full"
            />
            
            <!-- Calendar Date Picker -->
            <div class="relative flex items-center">
              <input type="date" :min="minDate" v-model="newTaskDueDate" class="text-xs text-gray-300 bg-[#16233B] border border-white/10 rounded px-1.5 py-1 focus:outline-none focus:border-blue-400 cursor-pointer" />
            </div>
          ...
          <button @click="handleAddCard" class="p-1 text-gray-400 hover:text-blue-400 border border-white/10 hover:border-blue-400 rounded transition-colors flex items-center justify-center">
