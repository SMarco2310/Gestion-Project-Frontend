...
const statusMetrics = computed(() => {
  ...
  return [
    { label: 'À faire', percentage: `${Math.round((todo / total) * 100)}%`, colorClass: 'bg-[#FFB78C]' },
    { label: 'En cours', percentage: `${Math.round((inProgress / total) * 100)}%`, colorClass: 'bg-[#8CA8F9]' },
    { label: 'Terminées', percentage: `${Math.round((done / total) * 100)}%`, colorClass: 'bg-[#A6C4FF]' },
  ]
})
...
<template>
    <div>
        ...
        <p class="text-secondary dark:text-gray-500 text-sm md:text-md pt-3">Bienvenue sur votre tableau de bord, {{ userName }}</p>
    </section>
    <br>
    <section>
