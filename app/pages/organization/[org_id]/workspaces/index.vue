...
onMounted(async () => {
  ...
  continueTourIfPending('workspaces')
})
...
<template>
  <div>
    <!-- Header -->
    <section id="tour-workspaces-header" class="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-10">
      <div>
      ...
      <button id="tour-create-workspace-btn" @click="isCreateWorkspaceModalOpen = true" class="px-5 py-2.5 btn-primary text-white font-medium rounded-xl hover:btn-primary hover:brightness-90 transition-colors flex items-center justify-center gap-2 shadow-sm shrink-0">
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      <!-- Workspace Cards -->
      <div v-for="ws in workspaces" :key="ws.id" class="bg-card dark:bg-[#1A1A1D] rounded-2xl p-6 border border-gray-200 dark:border-gray-800 flex flex-col hover:border-primary/30 transition-colors group min-h-[250px] shadow-sm">
        ...
        <div class="flex items-center gap-3">
          <NuxtLink :to="`/organization/${orgId}/workspace/${ws.id}`" class="flex-1 py-2.5 rounded-xl font-bold text-sm text-center transition-colors shadow-sm" :class="activeWorkspace?.id === ws.id ? 'bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400 cursor-not-allowed' : 'btn-primary text-white hover:brightness-110'">
